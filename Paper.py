...
if not(os.path.exists('./Output/res_HSBM-woutDC-MCMC.pkl')):
...
if not(os.path.exists('./Output/res_HSBM-withDC-MCMC.pkl')):
    if os.path.exists('./Output/res_HSBM-withDC-temp.pkl'):
        with open('./Output/res_withDC-temp.pkl', "rb") as file:
            res_withDC = pickle.load(file)
        i_start =  100 # Run 1 on 2026/01/30
        print(f"Temporary result loaded")
    else:
    ...
    MDL = res_withDC.entropy()
    print(f"Starting from {i_start}")
    for i in range(i_start, 10000): 
        seed_rng(78213+i) # Set a seed for the RNG
        res_withDC.multiflip_mcmc_sweep(niter=100)
        if(res_withDC.entropy()<MDL):
            print(f"Sweep {i}: Description length = {res_withDC.entropy():.4f}")
            MDL = res_withDC.entropy()
        if i % 100 == 0:
            with open(f'./Output/res_withDC-{i}.pkl', 'wb') as file:
                pickler = pickle.Pickler(file)
                pickler.dump(res_withDC)
